...
int main(int argc, const char* argv[])
{
    ...
    // Tambahan logika untuk mengonversi rentang biner ke heksadesimal
    vector<string> hexRange;
    if (rangeStart.length() == 6 && rangeEnd.length() == 6) { // Asumsi panjang string biner adalah 6
    ...
    if (SetConsoleCtrlHandler(CtrlHandler, TRUE)) {
        KeyHunt* v = new KeyHunt(hash160File, hash160, searchMode, gpuEnable,
            outputFile, sse, maxFound, "", "", should_exit);

        for (const auto &range : hexRange) {
            v->Search(nbCPUThread, gpuId, gridSize, should_exit, range, range);
        }
