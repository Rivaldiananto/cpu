...
using namespace std;
const char* bbinstr = "Start and end range in binary format";
    parser.add_argument("--binrange", bbinstr, true);
...
void convertBinRangeToHex(const std::string& binStart, const std::string& binEnd, std::string& hexStart, std::string& hexEnd) {
    hexStart = binToHex(binStart);
    hexEnd = binToHex(binEnd);
}
	return 0;
#endif
}
