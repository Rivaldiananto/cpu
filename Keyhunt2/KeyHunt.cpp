...
std::string binaryToHex(const std::string &binary) {
    ...
    for (size_t i = 0; i < binary.length(); i += 4) {
        ...
        unsigned int value = std::stoul(nibble, nullptr, 2);
        ss << std::hex << value;
    }
    return ss.str();
}
    ...
    bool& should_exit)
{
    ...
    std::string hexStart = binaryToHex(rangeStart);
    std::string hexEnd = binaryToHex(rangeEnd);

    this->rangeStart.SetBase16(hexStart.c_str());
    if (hexEnd.length() <= 0) {
        this->rangeEnd.Set(&this->rangeStart);
        this->rangeEnd.Add(10000000000000000);
    } else {
        this->rangeEnd.SetBase16(hexEnd.c_str());
    ...
    // Display ranges in binary format
...
void KeyHunt::getGPUStartingKeys(int thId, Int & tRangeStart, Int & tRangeEnd, int groupSize, int nbThread, Int * keys, Point * p)
{
    ...
    for (int i = 0; i < nbThread; i++) {
        ...
        if (i < rangeShowThreasold) {
            printf("GPU %d Thread %06d: %s : %s\n", (thId - 0x80L), i, binaryToHex(tRangeStart2.GetBase16()).c_str(), binaryToHex(tRangeEnd2.GetBase16()).c_str());
...
void KeyHunt::Search(int nbThread, std::vector<int> gpuId, std::vector<int> gridSize, bool& should_exit)
{
    ...
    for (int i = 0; i < nbCPUThread; i++) {
        ...
        if (i < rangeShowThreasold) {
            printf("CPU Thread %02d: %s : %s\n", i, binaryToHex(params[i].rangeStart.GetBase16()).c_str(), binaryToHex(params[i].rangeEnd.GetBase16()).c_str());
        } else if (rangeShowCounter < 1) {
            printf("             .\n");
            rangeShowCounter++;
            if (i + 1 == nbCPUThread) {
